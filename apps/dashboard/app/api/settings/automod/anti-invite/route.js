...
export async function POST(request) {
 try {
   ...
   data.actions = [
    {
     ...
     metadata: {
      custom_message: "Message blocked due to containing an invite link. Rule added by Kepler",
     },
    },
   ];
  }
  ...
  const validatedActions = await validateAutoModRuleActions(data.actions, allChannels, "Message blocked due to containing an invite link. Rule added by Kepler");
  ...
  const createdRule = await createHTTPAutomodRule(server.id, "anti-invite", {
   ...
   name: "Disallow invites [keplerbot.xyz]",
