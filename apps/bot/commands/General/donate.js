...
export default {
 name: "donate",
 description: "🪙 Help us develop Kepler by donating",
  ...
  try {
   if (!client.config.donate.enabled || !client.config.donate.links) {
    const embed = new EmbedBuilder()
     ...
     .setTimestamp()
     .setTitle("🪙 Donate to Kepler");
    return interaction.followUp({ ephemeral: false, embeds: [embed] });
   }

   const embed = new EmbedBuilder()
    .setDescription("> **You can donate to Kepler's developer by using the following methods:**\n" + client.config.donate.links.map((link) => `- [${link.icon} ${link.name}](${link.url})`).join("\n"))
    .setFooter({
     text: `Requested by ${interaction.member.user.globalName || interaction.member.user.username}`,
     iconURL: interaction.member.user.displayAvatarURL({
      size: 256,
