...
export default {
 name: "contact",
 description: "📝 Contact the Kepler team",
 ...
 run: async (client, interaction, guildSettings) => {
  try {
   ...
   const embed = new EmbedBuilder()
    .setDescription(`Click the button below or [click here](${client.config.url}/contact) to contact the Kepler team.\n\n>>> **Useful links:**\n- [View all Kepler commands](${client.config.url}/commands)\n- [Kepler support server](${client.config.url}/support)`)
    .setFooter({
     text: `Requested by ${interaction.member.user.globalName || interaction.member.user.username}`,
     iconURL: interaction.member.user.displayAvatarURL({
      size: 256,
