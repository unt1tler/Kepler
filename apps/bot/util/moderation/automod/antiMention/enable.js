...
import { createAutoModRule, syncAutoModRule } from "@kepler/util/database";
...
export async function enableAntiMention(client, interaction, guildSettings) {
 ...
 if (createdRule) {
  ...
  const ruleToCreate = {
   name: "Disallow mention spam [keplerbot.xyz]",
   creatorId: client.id,
   enabled: true,
   ...
   actions: [
    {
     ...
     metadata: {
      ...
      customMessage: "Message blocked due to containing too many mentions. Rule added by Kepler",
     },
    },
   ],
  ...
  if (logChannel) {
   ...
   ruleToCreate.actions.push({
    ...
    metadata: {
     channel: logChannel,
     message: "Message blocked due to containing too many mentions. Rule added by Kepler",
    },
   });
  }
