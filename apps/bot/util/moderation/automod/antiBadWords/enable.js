...
export async function enableAntiBadWords(client, interaction, guildSettings) {
 ...
 if (createdRule) {
  ...
  const ruleToCreate = {
   name: "Anti bad words [keplerbot.xyz]",
   creatorId: client.id,
   enabled: true,
   ...
   actions: [
    {
     ...
     metadata: {
      ...
      customMessage: "Message blocked due to containing a bad word. Rule added by Kepler",
     },
    },
   ],
  ...
  if (logChannel) {
   ...
   ruleToCreate.actions.push({
    ...
    metadata: {
     channel: logChannel,
     message: "Message blocked due to containing a bad word. Rule added by Kepler",
    },
   });
  }
